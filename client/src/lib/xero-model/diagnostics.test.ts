...
describe('diagnostics contract', () => {
  it('accepts strict diagnostic checks and rejects invalid severity/state combinations', () => {
    const passed = xeroDiagnosticCheckSchema.parse({
      contractVersion: 1,
    ...
    expect(() =>
      xeroDiagnosticCheckSchema.parse({
        ...passed,
        status: 'passed',
        severity: 'error',
        retryable: true,
  ...
  it('creates redacted diagnostics for provider repair details without leaking secrets', () => {
    const sanitized = sanitizeDiagnosticText(
      'Read failed at /Users/sn0w/.config/xero/provider.json with Authorization: Bearer sk-live-secret',
    )
    ...
    const diagnostic = createXeroDiagnosticCheck({
  ...
  it('redacts opaque auth headers, cloud credential paths, and nested copied doctor payloads', () => {
    ...
    const copiedJson = renderXeroDoctorReport({
      contractVersion: 1,
  ...
  it('builds doctor reports with stable summaries, skipped checks, and copy-safe output modes', () => {
    const failed = createXeroDiagnosticCheck({
      subject: 'settings_dependency',
      status: 'failed',
      severity: 'error',
      retryable: false,
      ...
      message:
        'Settings read failed at /Users/sn0w/Library/Application Support/dev.sn0w.xero/secrets.json with token=sk-live-secret',
      remediation: 'Move the secret out of copied diagnostics and resave settings.',
    })
    const skipped = createXeroDiagnosticCheck({
    ...
    const passed = createXeroDiagnosticCheck({
      subject: 'runtime_supervisor',
      status: 'passed',
      severity: 'info',
      retryable: false,
    ...
    const report = createXeroDoctorReport({
