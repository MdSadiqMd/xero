...
export function getAgentSessionUnavailableCredentialReason(
  ...
  switch (runtimeSession.phase) {
    ...
    case 'awaiting_browser_callback':
      return `Cadence is waiting for the ${providerLabel} browser callback to return.`
    case 'awaiting_manual_input':
      return `Cadence is waiting for the pasted ${providerLabel} redirect URL to finish login.`
    case 'starting':
      return `Cadence is opening the ${providerLabel} login flow.`
    case 'exchanging_code':
      return `Cadence is completing the ${providerLabel} bind.`
    case 'refreshing':
      return `Cadence is refreshing the ${providerLabel} session.`
    case 'idle':
      return `Bind ${providerLabel} to create or refresh the runtime session.`
    case 'cancelled':
      return `The ${providerLabel} bind was cancelled before Cadence could create a runtime session.`
    case 'failed':
      return `Cadence could not bind ${providerLabel} for this project.`
  }
}
...
export function getAgentMessagesUnavailableCredentialReason(
  ...
  if (agentRuntimeBlocked) {
    return runtimeRun
      ? 'Cadence recovered durable supervised-run state, but live streaming requires a provider credential. Add one in Settings to resume the stream.'
      : 'Add a provider credential in Settings to start the runtime session and live transcript.'
  }

  if (!runtimeSession) {
    return runtimeRun
      ? 'Cadence recovered durable supervised-run state, but live streaming requires a desktop-authenticated runtime session.'
      : 'Bind a provider in Settings to establish a runtime session for this project.'
  }

  if (!runtimeSession.isAuthenticated) {
    if (runtimeSession.isLoginInProgress) {
      return 'Cadence is binding the selected provider. Wait for the saved-key validation to finish before expecting live stream activity.'
    }
    return runtimeRun
      ? 'Cadence recovered durable supervised-run state, but live streaming still requires an authenticated runtime session.'
      : 'Bind a provider in Settings to establish a runtime session for this project.'
  }
