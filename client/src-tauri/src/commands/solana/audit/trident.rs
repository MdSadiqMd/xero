...
fn cargo_toml_body(target: &str) -> String {
    format!(
        r#"# Generated by Xero for Trident fuzzing.
[package]
...
fn trident_toml_body() -> &'static str {
    r#"# Xero-generated Trident workspace descriptor.
...
fn fuzz_entry_body(target: &str, instructions: &[String]) -> String {
    ...
    if instructions.is_empty() {
        ix_lines.push_str(
            "    // TODO(xero): add one InstructionAccount arm per IDL instruction.\n    // See https://ackee.xyz/trident for the fuzz attribute DSL.\n",
        );
    } else {
    ...
    format!(
        r#"//! Xero-generated Trident fuzz harness for `{target}`.
//!
