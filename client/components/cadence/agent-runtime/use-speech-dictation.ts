...
import type {
  ...
  CadenceDictationSession,
} from '@/src/lib/cadence-desktop'
import type {
  DictationEventDto,
  DictationStatusDto,
} from '@/src/lib/cadence-model/dictation'
import type { OperatorActionErrorView } from '@/src/features/cadence/use-cadence-desktop-state/types'
...
export function useSpeechDictation({
  ...
  const sessionRef = useRef<CadenceDictationSession | null>(null)
  ...
  const handleChannelError = useCallback(
    (channelError: unknown) => {
      setError(toDictationError(channelError, 'Cadence could not read the native dictation stream.'))
      releaseSession()
      updatePhase('idle')
    },
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      releaseSession()
      setError(toDictationError(startError, 'Cadence could not start native dictation.'))
      updatePhase('idle')
    }
  }, [
    focusPromptInput,
  ...
  const stop = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setError(toDictationError(stopError, 'Cadence could not stop native dictation.'))
      releaseSession()
      updatePhase('idle')
      return false
    }
  ...
  const cancel = useCallback(async () => {
    ...
    try {
      ...
      setError(toDictationError(cancelError, 'Cadence could not cancel native dictation.'))
    } finally {
      releaseSession()
      updatePhase('idle')
    }
