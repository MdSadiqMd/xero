...
import type { CloudProviderPreset } from "@/src/lib/xero-model/provider-presets"
import {
...
function createDraft(credential: ProviderCredentialDto | null): CredentialDraft {
  return {
    apiKey: "",
...
function buildUpsertRequest(
  ...
  switch (preset.authMode) {
    ...
    case "oauth":
      throw new Error(
        `Xero persists ${preset.providerId} credentials through OAuth, not the upsert command.`,
      )
  }
}
...
export function ProviderCredentialsList({
  ...
  const ensureDraft = (
    ...
    setDrafts((prev) =>
      prev[providerId] !== undefined
        ? prev
        : { ...prev, [providerId]: createDraft(credential) },
    )
  }
  ...
  const handleToggle = (
    ...
    ensureDraft(providerId, credential)
  ...
  const handleSave = async (preset: CloudProviderPreset) => {
    ...
    const draft = drafts[providerId] ?? createDraft(findCredential(providerCredentials, providerId))
    ...
    try {
      ...
      setSaveError({
        providerId,
        message: errMsg(error, "Xero could not save the provider credential."),
      })
    }
  }
  ...
  const handleDelete = async (providerId: SupportedProviderId) => {
    ...
    try {
      ...
      setSaveError({
        providerId,
        message: errMsg(error, "Xero could not remove the provider credential."),
      })
    }
  }
    ...
    try {
      ...
      if (session?.authorizationUrl) {
        try {
          ...
          setOpenAuthError({
            providerId,
            message: errMsg(urlError, "Xero could not open the browser for sign-in."),
          })
        }
      }
    } catch (error) {
      setOpenAuthError({
        providerId,
        message: errMsg(error, "Xero could not start the sign-in flow."),
      })
    } finally {
      setAuthPending(null)
    }
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            {isOAuth ? (
              ...
              <Button
                ...
                onClick={() => handleToggle(providerId, credential)}
      ...
      {showLoadError ? (
        <Alert variant="destructive" className="border-destructive/40">
          ...
          <AlertDescription>
            {errorViewMessage(
              providerCredentialsLoadError,
              "Xero could not load provider credentials.",
            )}
          </AlertDescription>
        </Alert>
      ) : null}
